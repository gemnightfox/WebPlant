...
**Inviting users by email** — the invite hides whether the email is registered (security feature). If the invited user's `workspace_invites` preference is `False`, the invite is silently skipped. Role assignment requires `can_assign_roles_to_users`; otherwise `default_role` is used.
