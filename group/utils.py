...
def get_group_or_404(my_user, group_id):
    group = get_object_or_404(Group, id=group_id)
    get_workspace_or_404(my_user=my_user, workspace_id=group.project.workspace.id) # Verification purposes only (gives error if checks fail, eg. user not in workspace)
    return group
